...
# helpers migrated from timm
def to_2tuple(x):
    return (x, x) if not isinstance(x, tuple) else x


def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2., max_iters=8):
    """
    更接近 timm 的截断正态：反复采样，直到全部落在 [a, b] 或达到迭代上限。
    迭代上限足够大时与原实现接近，防止极端情况下卡死。
    """
    # 使用 randn + 仿射，避免 jittor.normal 的参数歧义
    out = jt.randn(tensor.shape) * std + mean
    mask = (out < a) | (out > b)
    iters = 0
    while mask.any() and iters < max_iters:
        resample = jt.randn(out.shape) * std + mean
        out = jt.where(mask, resample, out)
        mask = (out < a) | (out > b)
        iters += 1
    out = jt.clamp(out, a, b)
    tensor.assign(out)
    return tensor
...
class Padding_tensor(nn.Module):
    ...
    def execute(self, x):
        ...
        # padding size must be Python int (int64) for jittor.nn.pad
        h_padding = int(np.abs(h - h_patches * self.patch_size))
        w_padding = int(np.abs(w - w_patches * self.patch_size))
...
class Recons_tensor(nn.Module):
    ...
    def execute(self, patches_tensor, patches_paddings):
        ...
        patches_tensor = patches_tensor.reshape(-1, C, N, self.patch_size, self.patch_size)
        # ----------------------------------------
        pic_all = None
        for i in range(h_patches):
            pic_c = None
            for j in range(w_patches):
                if j == 0:
                    ...
                    pic_c = jt.concat((pic_c, patches_tensor[:, :, i * w_patches + j, :, :]), dim=3)
            if i == 0:
                pic_all = pic_c
            else:
                pic_all = jt.concat((pic_all, pic_c), dim=2)
...
class Attention(nn.Module):
    ...
    def execute(self, x):
        ...
        attn = nn.softmax(dp, dim=-1)  # (n_samples, n_heads, n_patches, n_patches)
        ...
        # if self.cross:
        #     x_temp = x.reshape(1, 256, 128, 2, 2).permute(0, 2, 1, 3, 4)
...
class Block(nn.Module):
    ...
    def execute(self, x):
        if self.cross:
            # Cross-attention should update the query branch (x[0]) using keys/values from x[1]/x[2]
            q, k, v = x
            q_norm = self.norm1(q)
            k_norm = self.norm1(k)
            v_norm = self.norm1(v)
            ...
            q_out = q + attn_out
            q_out = q_out + self.mlp(self.norm2(q_out))
            # Keep k/v unchanged to mirror the PyTorch behaviour
            out = [q_out, k, v]
        else:
...
class cross_atten_module(nn.Module):
    ...
    def execute(self, x1_ori, x2_ori):
        # Apply positional dropout to both branches for symmetry
        q = self.pos_drop(x1_ori)
        k = self.pos_drop(x2_ori)
        v = k
        x = [q, k, v]
        for block in self.blocks:
            x = block(x)
            # Normalize the updated query branch (same as PyTorch impl)
            x[0] = self.norm(x[0])
        x_self = x[0]
        return x_self
...
class self_atten(nn.Module):
    ...
    def execute(self, x1, x2, last=False):
        # patch
        ...
        # b, n, c*h*w
        # 对齐 PyTorch: transpose(2, 1) 将 (b, c, n, h, w) -> (b, n, c, h, w)，然后 reshape
        # 注意：Jittor 的 transpose 参数顺序与 PyTorch 相同，都是 (dim1, dim2)
        x_patched1 = x_patched1.transpose(1, 2).reshape(b, n, c * h * w)
        x_patched2 = x_patched2.transpose(1, 2).reshape(b, n, c * h * w)
        x1_self_patch = self.self_atten1(x_patched1)
        x2_self_patch = self.self_atten2(x_patched2)
       
        # reconstruct
        if last is False:
            x1_self_patch = x1_self_patch.reshape(b, n, c, h, w).permute(0, 2, 1, 3, 4)
            x_self1 = self.recons_tensor(x1_self_patch, patches_paddings)  # B, C, H, W
            x2_self_patch = x2_self_patch.reshape(b, n, c, h, w).permute(0, 2, 1, 3, 4)
...
class cross_atten(nn.Module):
    ...
    def execute(self, x1, x2, patches_paddings):
        # patch
        x_patched1, patches_paddings = self.patch_embed_tensor(x1)
        # B, C, N, Ph, Pw = x_patched1.shape
        x_patched2, _ = self.patch_embed_tensor(x2)
        ...
        b, c, n, h, w = x_patched1.shape
        # b, n, c*h*w
        # 对齐 PyTorch: transpose(2, 1).contiguous().view() -> transpose(1, 2).reshape()
        x1_self_patch = x_patched1.transpose(1, 2).reshape(b, n, c * h * w)
        x2_self_patch = x_patched2.transpose(1, 2).reshape(b, n, c * h * w)
        ...
        cross1 = cross1.reshape(b, n, c, h, w).permute(0, 2, 1, 3, 4)
        cross1_all = self.recons_tensor(cross1, patches_paddings)  # B, C, H, W
        
        cross2 = cross2.reshape(b, n, c, h, w).permute(0, 2, 1, 3, 4)
        cross2_all = self.recons_tensor(cross2, patches_paddings)  # B, C, H, W
        
        out = out.reshape(b, n, c, h, w).permute(0, 2, 1, 3, 4)
...
class cross_encoder(nn.Module):
    ...
    def _init_weights(self):
        """Match timm ViT init: trunc_normal on Linear, ones/zeros on LayerNorm."""
